...
class AlarmItem: Object {
    ...
    //作業終了時間 - 作業開始時間の差分
    @objc dynamic var byItemDifferentTime: TimeInterval = 0
    //ユーザーが設定した項目名
    @objc dynamic var userSetupName: String = ""
    
    //初期化(Realmを使用する引数Objectもクラスは初期化時にconvinienceをつける)
    convenience init(alarmSettingId: String, id: String, masterId: String, byItemStartTime: Date, byItemEndTime: Date, byItemDifferentTime: TimeInterval, userSetupName: String) {
        self.init()
        ...
        self.byItemDifferentTime = byItemEndTime.timeIntervalSince(byItemStartTime)
